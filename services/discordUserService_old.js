  ...
  async postAsUser(userId, scanData, userMessage = '') {
    try {
      ...
      console.log('🔍 Database query result:', {
        ...
        hasDiscordUserId: userResult.rows[0]?.discord_user_id ? 'Yes' : 'No',
        hasAccessToken: userResult.rows[0]?.discord_access_token ? 'Yes' : 'No',
        tokenLength: userResult.rows[0]?.discord_access_token?.length || 0
      });
      ...
      if (!discord_access_token) {
        throw new Error('Discord access token not found in database');
      }
      ...
      console.error('Channel ID:', scanData.isHealthy ? process.env.DISCORD_HEALTHY_CHANNEL_ID : process.env.DISCORD_SICK_CHANNEL_ID);
      console.error('User ID:', userId);
      console.error('Has Access Token:', !!discord_access_token);
      console.error('Token Length:', discord_access_token ? discord_access_token.length : 0);
